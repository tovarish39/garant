...
pid_file_path = "#{ROOT_GARANT}/tmp/2_secondary.pid"
File.delete(pid_file_path) if File.exist?(pid_file_path)
File.open(pid_file_path, 'a') { |pid_file| pid_file.puts Process.pid }


log_path = "#{ROOT_GARANT}/logs/2_secondary.log"
...
Telegram::Bot::Client.run(Bot_Token_2_SECONDARY) do |bot|
  bot.listen do |message|
    ...
    begin
      ...
      Send.mes(text: e, chat_id: CHAT_ID_MY)
      Send.mes(text: e.backtrace, chat_id: CHAT_ID_MY)
    end
  end
end
