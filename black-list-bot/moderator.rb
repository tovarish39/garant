...
pid_file_path = "#{ROOT_BOT}/tmp/moderator_pid.txt"
File.delete(pid_file_path) if File.exist?(pid_file_path)
File.open(pid_file_path, 'a') { |pid_file| pid_file.puts Process.pid }

$logger = Logger.new("#{ROOT_BOT}/log/moderator.log", 'weekly')
$logger.formatter = proc do |severity, datetime, _progname, msg|
  date_format = datetime.strftime('%Y-%m-%d %H:%M:%S')
  "#{severity.slice(0)} date=[#{date_format}] pid=##{Process.pid} message='#{msg}'\n"
end

Telegram::Bot::Client.run(TOKEN_BOT_MODERATOR) do |bot|
  bot.listen do |message|
    $mes = message
    $bot = bot

    begin
      handle_moderator if $mes.from.present?
