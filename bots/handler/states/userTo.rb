...
def choose_role
  ...
  edit_message(B_choose_role[$lang], IM_role.call)
end
#######################################################################
...
def choose_type_of_currencies 
  return if !get_userTo() # defining $userTo
  edit_message(B_currency_types[$lang], IM_currency_types.call)
...
def choose_specific_currency
  return if !get_userTo() # defining $userTo
  edit_message(B_currency_types[$lang], IM_cryptocurrencies.call)
end
...
def choose_amount
  ...
  send_message("#{B_push_amount_currency[$lang]} <b>#{selected_currency}</b> " )
end
...
def view_type_of_disputs
  ...
  edit_message(B_disputs_by_userTo[$lang], IM_type_of_disputs.call)
end
