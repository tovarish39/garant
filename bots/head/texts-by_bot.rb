...
B_deal_status = lambda {
  ...
  if status.nil? # "Запросы"
    B_pending_userTo[$lg]
  elsif status =~ /accessed/              # "Запросы"
    B_pending_userTo[$lg]
  elsif status =~ /payed/                 # 'Активные'
  elsif status =~ /dispute/               # "Споры"
    B_opened_by[$lg] + created_by + (is_in_process ? "\n #{B_leads[$lg]} @#{dispute.moderator.username}" : '').to_s
  elsif status == 'canceled by_seller'    # "История сделок"
    B_canceled_by_seller[$lg]
  elsif status == 'finished by_custumer'  # "История сделок"
    B_finished_by_custumer[$lg]
  elsif status == 'finished by_moderator' # "История сделок"
    B_finished_by_moderator.call # + B_funds_to_custumer[$lg]
  elsif status == 'rejected by_seller' # "История сделок"
    B_rejected_by_seller[$lg]
  elsif status == 'rejected by_custumer' # "История сделок"
    B_rejected_by_custumer[$lg]
  elsif status == 'finished by_administrator' # "Завершена администратором"
    "#{B_finished_by_admin[$lg]}\n#{$comment_by_administrator}\n#{B_funds_to_seller[$lg]}"
  elsif status == 'canceled by_administrator' # "Отменена администратором"
    "#{B_canceled_by_admin[$lg]}\n#{$comment_by_administrator}\n#{B_funds_to_custumer[$lg]}"
  elsif status == 'garant win by_administrator' # "в пользу гаранта администратором"
