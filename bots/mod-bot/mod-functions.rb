...
def handle_acception
    ...
    dispute.sended_to_moderators.each do |obj|
        ...
        begin
            edit_message("спор по сделки ##{dispute.deal.hash_name} взят вами в обработку", nil, mes_id)                if mod_id == $mod.id.to_s
            edit_message("спор по сделки ##{dispute.deal.hash_name} взят другим модератором в обработку", nil, mes_id)  if mod_id != $mod.id.to_s
            $logger.info("#{obj} dispute_id #{dispute.id} ---> edited successfull")
        rescue => exception
            $logger.error("#{dispute.sended_to_moderators.inspect} ---> #{exception}")
        end
...
def list_my_inProcess_disputes
    ...
    elsif inProcess_disputes.size == 0
...
def finishing_dispute
    if    text_mes?('Отмена')
        ...
        send_message("спор по сделке ##{dispute.deal.hash_name} обработан",  RM.call(['Открытые споры', 'Мои споры', "История споров"]))
...
def list_my_closed_disputs
    ...
    if finished_disputes.size != 0
        finished_disputes.each do |dispute|
            data = get_data_dispute(dispute)
            send_message(
                B_disput_offer.call(data[:seller], data[:custumer], data[:deal], dispute, data[:initiator], Ru) + B_dispute_comment.call(dispute),
              )
        end
    elsif finished_disputes.size == 0
